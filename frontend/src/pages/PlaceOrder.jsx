    ...
    <div className='flex flex-col sm:flex-row justify-between gap-4 pt-4 sm:pt-14 min-h-[80vh]  border-t '>
      ...
      <div className='mt-8 '>
        ...
        <div className='mt-12'>
          ...
          {/* PAYMENTMETHODE SELECTION */}
          <div className='flex gap-3 flex-col lg:flex-row'>
             <div onClick={()=>setMethode('stripe')} className='flex items-center gap-3 border p-2 px-2 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${methode === 'stripe' ? 'bg-green-400' : ''} `}></p>
              <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />
             </div>
             <div onClick={()=>setMethode('razorpay')} className='flex items-center gap-3 border p-2 px-2 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${methode === 'razorpay'  ? 'bg-green-400' : '' } `}></p>
              <img className='h-5 mx-4' src={assets.razorpay_logo} alt="" />
             </div>
             <div onClick={()=>setMethode('cod')} className='flex items-center gap-3 border p-2 px-2 cursor-pointer'>
